...
def embed(text):
    try:
        r = requests.post(f"{OLLAMA}/api/embeddings", json={"model": EMBED_MODEL, "prompt": text[:8000]}, timeout=60)
...
def collect_files(repo):
    ...
    for root, dirs, names in os.walk(repo):
        dirs[:] = [
            ...
            for d in dirs
            if d
            not in {"node_modules", ".next", "dist", ".git", "coverage", ".turbo", ".vercel", "playwright-report", "test-results", "build"}
        ]
        for name in names:
...
def main():
    ...
    if args.delete_existing:
        ...
        del_body = {
            "filter": {
                "must": [{"key": "project", "match": {"value": args.project}}, {"key": "source", "match": {"any": ["code", "sql", "doc"]}}]
            }
        }
    ...
    for i in range(0, len(points), 100):
        r = requests.put(f"{QDRANT}/collections/{COLLECTION}/points?wait=true", json={"points": points[i : i + 100]}, timeout=60)
        if i % 500 == 0:
