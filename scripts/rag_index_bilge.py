...
def create_collection():
    ...
    r = requests.put(f"{QDRANT}/collections/{COLLECTION}", json={"vectors": {"size": 768, "distance": "Cosine"}})
...
def upsert_batch(points):
    ...
    if not points:
        return
    r = requests.put(f"{QDRANT}/collections/{COLLECTION}/points?wait=true", json={"points": points}, timeout=60)
            ...
            all_points.append(
                {
                    "id": str(uuid.uuid4()),
                    "vector": vec,
                    "payload": {"source": "session", "date": date, "device": device, "session_id": sid, "text": text[:500]},
                }
            )
            counts["session"] += 1
            ...
            all_points.append(
                {
                    "id": str(uuid.uuid4()),
                    "vector": vec,
                    "payload": {"source": "task", "project": proj, "task": task, "status": status, "task_id": tid, "text": text[:500]},
                }
            )
            counts["task"] += 1
