...
cur.execute(
    "SELECT id, type, name, description, content FROM memories WHERE name LIKE ? OR description LIKE ? OR content LIKE ?",
    (f"%{FILTER}%",) * 3,
)
        ...
        points.append(
            {
                "id": str(uuid.uuid4()),
                "vector": vec,
                "payload": {"source": "memory", "type": typ, "name": name, "memory_id": mid, "text": text[:2000]},
            }
        )
        ...
        points.append(
            {
                "id": str(uuid.uuid4()),
                "vector": vec,
                "payload": {"source": "discovery", "type": typ, "title": title, "status": status, "discovery_id": did, "text": text[:2000]},
            }
        )
        ...
        points.append(
            {
                "id": str(uuid.uuid4()),
                "vector": vec,
                "payload": {"source": "session", "date": date, "device": dev, "session_id": sid, "text": text[:2000]},
            }
        )
        ...
        points.append(
            {
                "id": str(uuid.uuid4()),
                "vector": vec,
                "payload": {"source": "task", "project": proj, "task": task, "status": status, "task_id": tid, "text": text[:2000]},
            }
        )
...
for i in range(0, len(points), 100):
    requests.put(f"{QDRANT}/collections/{COLLECTION}/points?wait=true", json={"points": points[i : i + 100]}, timeout=60)
