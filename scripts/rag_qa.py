...
def search(q, k=4):
    v = embed(q)
    return requests.post("http://localhost:6333/collections/bilge-arena/points/search", json={"vector":v,"limit":k,"with_payload":True}).json().get("result", [])
...
def rag_qa(query):
    ...
    context = "\n\n".join([f"--- Kaynak {i+1} ({h['payload']['source']}, skor {h['score']:.2f}) ---\n{h['payload'].get('text', '')[:1200]}" for i, h in enumerate(hits)])
