...
def _fake_bins(bindir: Path, capture: Path) -> None:
...
def _run(tmp_path: Path, result_json: str, prev_state: str | None = None) -> tuple[str, str]:
    capture = tmp_path / "cap.log"
    _fake_bins(tmp_path / "bin", capture)
