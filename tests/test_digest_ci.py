...
def _run(run_id, timestamp, total, passed, failed, details):
    return {"id": run_id, "timestamp": timestamp, "total": total, "passed": passed, "failed": failed, "details": details}
    ...
    path = _make_coverage_db(
        ...
        [
            _run(
                ...
                2396,
                0,
                {"panola": {"status": "pass", "passed": 843, "failed": 0}, "klipper": {"status": "pass", "passed": 777, "failed": 0}},
            )
        ],
    )
    ...
    path = _make_coverage_db(
        ...
        [
            _run(
                ...
                95,
                5,
                {"panola": {"status": "pass", "passed": 50, "failed": 0}, "kuafor": {"status": "fail", "passed": 45, "failed": 5}},
            )
        ],
    )
    monkeypatch.setattr(core_digest, "COVERAGE_DB_PATH", path)
...
def test_ci_trend_regression_and_drop(monkeypatch, tmp_path):
    ...
    path = _make_coverage_db(
        tmp_path,
        [
            _run(61, today, 150, 150, 0, {"panola": {"passed": 100, "failed": 0}, "old-proj": {"passed": 50, "failed": 0}}),
