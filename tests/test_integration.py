...
# Use the TEST_API_KEY from conftest
...
@pytest.mark.anyio
async def test_kernel_status_flow(client):
...
@pytest.mark.anyio
async def test_kernel_governor_read(client):
...
@pytest.mark.anyio
async def test_system_info_flow(client):
...
@pytest.mark.anyio
async def test_process_list_flow(client):
    resp = await client.get("/api/v1/system/processes?limit=5")
    assert resp.status_code == 200
    data = resp.json()
    assert data["total"] <= 5
...
@pytest.mark.anyio
async def test_process_list_default_limit(client):
...
@pytest.mark.anyio
async def test_file_crud_cycle(client, tmp_path):
    ...
    try:
        ...
        # Create
        resp = await client.put("/api/v1/files/write", json={
            "path": test_path,
            "content": "hello world",
            "mode": "write",
        })
        ...
        # Read
        resp = await client.get(f"/api/v1/files/read?path={test_path}")
        assert resp.status_code == 200
        assert resp.json()["content"] == "hello world"

        # Edit
        resp = await client.patch("/api/v1/files/edit", json={
            "path": test_path,
            "old_string": "hello",
            "new_string": "goodbye",
        })
        ...
        # Delete
        resp = await client.delete(f"/api/v1/files/delete?path={test_path}")
        assert resp.status_code == 200
...
@pytest.mark.anyio
async def test_file_write_and_append(client, tmp_path):
...
@pytest.mark.anyio
async def test_file_read_unauthorized_path(client):
    """Reading a path outside allowed_paths must fail."""
    resp = await client.get("/api/v1/files/read?path=/etc/shadow")
...
@pytest.mark.anyio
async def test_monitor_metrics_flow(client):
...
@pytest.mark.anyio
async def test_monitor_metrics_types(client):
...
@pytest.mark.anyio
async def test_logs_sources(client):
...
async def test_agent_crud(client):
    ...
    # Create
    resp = await client.post("/api/v1/agents/create", json={
        "name": agent_name,
    ...
    # Delete
    resp = await client.delete(f"/api/v1/agents/{agent_name}")
    assert resp.status_code == 200
...
@pytest.mark.anyio
async def test_network_interfaces(client):
...
@pytest.mark.anyio
async def test_network_connections(client):
    resp = await client.get("/api/v1/network/connections")
    assert resp.status_code == 200
...
@pytest.mark.anyio
async def test_webops_services(client):
...
@pytest.mark.anyio
async def test_ssh_sessions_empty(client):
