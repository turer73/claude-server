...
# Fail-LOUD: key yoksa acik-log + graceful-skip (crash yerine). stderr'e: heredoc yalniz
# stderr'i LOG_FILE'a yonlendirir (2>>) — stdout'a yazilsa retry-log'a dusmez (Codex #261-P2).
...
if not _keys:
    print(f'MEMORY_API_KEY yok ({_env_path}) — poison memory-entry ATLANDI (Telegram-alert gonderildi)', file=sys.stderr)
    raise SystemExit(0)
KEY = _keys[0]
body = json.dumps({
    'type': 'project',
