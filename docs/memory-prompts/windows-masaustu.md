...
- **Auth Header:** `X-Memory-Key: REDACTED_MEMORY_KEY`
...
```bash
curl -s -H "X-Memory-Key: REDACTED_MEMORY_KEY" http://REDACTED_TAILSCALE_IP:8420/api/v1/memory/dashboard
curl -s -H "X-Memory-Key: REDACTED_MEMORY_KEY" "http://REDACTED_TAILSCALE_IP:8420/api/v1/memory/notes?device=windows-masaustu&unread_only=true"
```
...
```bash
curl -s -X POST http://REDACTED_TAILSCALE_IP:8420/api/v1/memory/notes \
  -H "Content-Type: application/json" \
  -H "X-Memory-Key: REDACTED_MEMORY_KEY" \
...
```bash
curl -s -X POST http://REDACTED_TAILSCALE_IP:8420/api/v1/memory/memories \
  -H "Content-Type: application/json" \
  -H "X-Memory-Key: REDACTED_MEMORY_KEY" \
...
7. **Arama** yapmak için:
```bash
curl -s -H "X-Memory-Key: REDACTED_MEMORY_KEY" "http://REDACTED_TAILSCALE_IP:8420/api/v1/memory/search?q=ANAHTAR_KELIME"
```
...
| Proje | URL | Stack |
|-------|-----|-------|
| Linux-AI Server | REDACTED_TAILSCALE_IP:8420 | FastAPI, SQLite, kernel |
