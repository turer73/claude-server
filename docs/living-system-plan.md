...
- ✅ **3.2 Hafif olay omurgası — TAMAMLANDI (2026-06-03), FAZ 3 KOMPLE:**
  - ✅ **Foundation** (PR #18 `a259650`): `server.db.events` + `app/core/events.py` + `scripts/emit-event.sh`. Codex 4 sessiz-arıza/crash fix.
  - ✅ **Producer-wiring (KAYIT-ONLY):** (a) job-outcome #19 `2751dd37` + (b) alerts-bridge #20 `14c6e98` + (c) VPS-backup-emit #21 `539daef`.
  - ✅ **(d) notify-cron CANLI** (PR #23 + #27 `95c44ef`): `events.pending` drain → **Telegram-direct** (n8n-workflow yok → surer adapte) → `mark_notified`. NOTIFY_CRON_ENABLED=true + crontab `*/20`. ATOMIK-cutover: cron-wrap n8n-POST guard'lı, backup-monitor `notify_backup` wrapper. SEND-THEN-MARK + LIMIT50 + OUTCOME-on-fail + read-fail-closed (Codex #23/#24). `_send_webhook` remediation KEEP (ayrı semantik).
