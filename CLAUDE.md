...
- Ollama host'ta (11434) — modeller: qwen3:30b-a3b-instruct-2507-q4_K_M (RAG default + reasoning, thinking-siz MoE — "default" etiketi fiktif, gerçek-tüketici bağlanana kadar açık-soru, topic-5 kararı), qwen3.5:9b (classify — 2026-07-12'de 2507'den taşındı), qwen2.5:3b (consciousness deep-thought), gemma3:12b-it-qat (TR-hi, GEÇİCİ — rolü DeepSeek-Layer2'ye taşınacak, topic-5 kararı P2/surer-lane tamamlanınca kaldırılır), qwen2.5:7b (KALICI — topic-5 kararıyla (2026-07-19) resmi "automation-lane" rolü verildi, sigorta-çerçevesi kalktı; 5-script bağımlılığı: autonomous-health-check.sh, autonomous-classifier-v2.sh, autonomous-spawn-summarize.sh, signal_quality.py, dispatch.py — LLMCore-DIŞI doğrudan Ollama çağrısı yapıyor, bu yüzden llm_calls telemetrisinde görünmüyor, LLMCore-migrasyonu surer P3 lane), bge-m3 + nomic-embed-text (embed). qwen3-coder:30b SİLİNDİ (2026-07-19, topic-5 kararı — 0-çağrı kanıtlı, .env LLM_ROUTE_CODE_REVIEW override'i kalıcı, code-review fiilen her zaman claude-haiku'ya gidiyordu; 4-model-önerisi qwen3.6:27b/r1:32b/r1:8b/openthinker:7b de aynı kararla reddedildi). Hibrit-thinking modeller (qwen3.5/3.6 ailesi) için `think:false` llmcore.py'de koşulsuz uygulanır (allowlist yok, güvenli-varsayılan)
