...
- Ollama host'ta (11434) — modeller: qwen3:30b-a3b-instruct-2507-q4_K_M (RAG default + reasoning, thinking-siz MoE), qwen3.5:9b (classify — 2026-07-12'de 2507'den taşındı), qwen2.5:3b (consciousness deep-thought), qwen3-coder:30b (code-review fallback), gemma3:12b-it-qat (TR-hi), qwen2.5:7b (SİLİNMEMELİ — 2026-07-18 canlı-denetimde 48h'de 118 çağrı bulundu: automation direct-curl lane'in görünmez-bağımlılığı — autonomous-health-check.sh, autonomous-classifier-v2.sh, autonomous-spawn-summarize.sh, signal_quality.py, dispatch.py LLMCore-DIŞI doğrudan Ollama çağrısı yapıyor, bu yüzden llm_calls telemetrisinde görünmüyordu. Eski "2026-07-19'da kaldırılabilir" notu YANLIŞTI/geri-alındı — karar topic-5'e taşındı, silme ÖNCESİ ya scriptler qwen3.5:9b'ye migrate edilmeli ya da model resmi automation-lane rosterine alınmalı), bge-m3 + nomic-embed-text (embed). Hibrit-thinking modeller (qwen3.5/3.6 ailesi) için `think:false` llmcore.py'de koşulsuz uygulanır (allowlist yok, güvenli-varsayılan)
